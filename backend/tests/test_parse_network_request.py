...
async def test_parse_network_request_incomplete_without_required_fields(monkeypatch):
    from app.config import settings; monkeypatch.setattr(settings, "mistral_api_key", None)
...
async def test_parse_network_request_does_not_retry_deterministic_parser_errors(monkeypatch):
    ...
    from pydantic import SecretStr; from app.config import settings; monkeypatch.setattr(settings, "mistral_api_key", SecretStr("test-key"))
