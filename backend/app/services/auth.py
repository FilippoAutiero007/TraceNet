...
def _validate_authorized_party(claims: dict[str, Any], _request: Request) -> None:
    """Verify that the authorized party (azp) claim matches configured allowed values."""
    configured = [
        item.strip()
    ...
    # Security: Do not fallback to the 'Origin' header, as it can be easily spoofed
    # by non-browser clients (e.g., curl, scripts), leading to auth bypass.
