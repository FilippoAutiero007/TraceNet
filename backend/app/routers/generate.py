...
@limiter.limit(os.environ.get("MAX_REQUESTS_PER_MINUTE", "10/minute"))
async def generate_pkt_file(
    ...
    try:
        ...
        output_base = os.environ.get("OUTPUT_DIR", "/tmp/tracenet")
...
async def download_file(request_id: str, filename: str):
    ...
    output_dir = Path(os.environ.get("OUTPUT_DIR", "/tmp/tracenet")) / request_id
...
async def health_check_router():
    ...
    checks["checks"]["mistral_api"] = "ok" if os.environ.get("MISTRAL_API_KEY") else "no_key"
