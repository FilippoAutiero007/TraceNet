...
# Clona il link e modifica FROM, TO, e PORT
...
if cable:
    ...
    # MODIFICA I PORT per router-2port
    ports = cable.findall('PORT')
    if len(ports) >= 2:
        ports[0].text = 'FastEthernet0/0'  # Porta Router0
        ports[1].text = 'FastEthernet0/0'  # Porta Router1
        print(f'PORT modificate: {ports[0].text}, {ports[1].text}')
    
    # MODIFICA TYPE a eCrossOver (Router-to-Router)
    type_elem = cable.find('TYPE')
    if type_elem is not None:
        type_elem.text = 'eCrossOver'
        print(f'TYPE modificato: {type_elem.text}')
...
output = Path('tmp_step1/test_full_link.pkt')
